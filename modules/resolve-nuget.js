"use strict";
(function () {
    const os = require("os"), debug = requireModule("debug")("resolve-nuget"), env = requireModule("env"), { chmodSync, writeFileSync, lsSync, fileExistsSync, FsEntities } = require("yafs"), which = requireModule("which"), path = require("path"), configGenerator = requireModule("resolve-nuget-config-generator"), toolsDir = requireModule("get-tools-folder")(), log = requireModule("log"), findNpmBase = requireModule("find-npm-base"), quoteIfRequired = requireModule("quote-if-required"), dotnetExe = process.platform === "win32" ? "dotnet.exe" : "dotnet", ZarroError = requireModule("zarro-error"), nugetExe = env.resolveFlag("DOTNET_CORE") ? dotnetExe : "nuget.exe";
