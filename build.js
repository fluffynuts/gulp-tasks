...
(function () {
    ...
    function buildForNetCore(solutions) {
        ...
        const configuration = env.resolve("BUILD_CONFIGURATION"), msbuildArgs = [];
    ...
    function buildAsStream(solutions) {
        ...
        if (env.resolveFlag("BUILD_SHOW_INFO")) {
            const buildRetries = env.resolveNumber("BUILD_RETRIES"), retryMessage = buildRetries > 0 ? `yes (${buildRetries})` : `no`;
